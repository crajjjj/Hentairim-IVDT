...
Event IVDTOnOrgasm(Form actorRef, Int thread)
	...
	printdebug("PC ORGASM! ")
		...
		;oninus lactis
		if HasOninusLactis() && utility.randomint(1,100) <= chancetolactateduringorgasm
			OninusLactislactate()
		endif
...
Bool Function ASLProcessStageAdvancing()
	if EnableAutoAdvanceStage != 1
		...
		return false
	ElseIf speeduppreventstageadvance == 1 && GetSpeedGear() > 0
		Printdebug("Stage not advancing due to : Speed up from Hentairim Game.")
		return false
...
Bool Function Isintense()
		return stringutil.find(Labelsconcat ,"1F") > -1 || IsGettingInsertedBig() ||  GetSpeedGear() >= speedgeartoincreaseintensity
endfunction
